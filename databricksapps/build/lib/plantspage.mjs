...
export function renderPlantsPage(plants, regions, outlined = new Set(), meta = {}) {
  ...
  const cynByIso = new Map(plants.filter(p => p.cyn).map(p => [p.cy, p.cyn]));
  const geoNamed = { ...geo, country: (cc) => (geo.country(cc) !== cc ? geo.country(cc) : (cynByIso.get(cc) || cc)) };
  const countryOf = (p) => geoNamed.country(p.cy);
  ...
  const regionRows = [...byReg.entries()]
    ...
    .map(([name, list]) => {
      const s = roll(list);
      const outl = list.filter(p => outlined.has(p.id)).length;
      return `<tr>
        <td><b>${esc(name)}</b></td>
        <td class="num">${n0(outl)}<span class="dim"> ${Math.round(outl / list.length * 100)}%</span></td>
        ...
        <td class="dim">${countryList(list, p => p.cy, geoNamed, 5)}</td>
      </tr>`;
    }).join('');
  ...
  const leavingRows = leaving.map(p => `<tr>
    <td><a href="/maps/plant/${esc(p.id)}"><b>${esc(p.n)}</b></a></td>
    <td>${esc(FUEL[p.f] || p.f)}</td>
    <td class="dim">${esc([p.st, BA_ISO[p.ba] || p.ba, countryOf(p)].filter(Boolean).join(' · '))}</td>
  ...
  <section class="panel card">
    <h2>By region</h2>
    ${table(['Region', { n: 'Perimeter' }, { n: 'Plants' }, { n: 'Operating MW' }, { n: 'Closing MW' },
  ...
  <section class="panel card">
    ...
    ${table(['Plant', 'Fuel', 'Where', { n: 'From' }, { n: 'MW leaving' }, { n: 'MW running' }, 'Footprint'],
  ...
  <section class="panel card">
    ...
    ${table(['Plant', 'Fuel', 'Where', { n: 'Closed' }, { n: 'MW was there' }, 'Footprint'],
  ...
  <section class="panel card">
    ...
    ${esc(meta.eia || '')}, a US Government work in the public domain. Everywhere else from
