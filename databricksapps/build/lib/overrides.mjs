...
// `suggest: true` marks the fields whose values recur across sites, so the
// edit form offers the registry's existing values as type-ahead suggestions
// (a datalist - anything new can still be typed). The point is convergence:
// the tenth Equinix site should say "Equinix" the way the first nine do,
// not invent an eleventh spelling.
export const FIELDS = {
  name:                   { label: 'Name' },
  operator:               { label: 'Operator', suggest: true },
  city:                   { label: 'City', suggest: true },
  address:                { label: 'Address' },
  country:                { label: 'Country', suggest: true,
  ...
  utility:                { label: 'Serving utility', suggest: true },
  // Coordinates, two ways: typed here when the right value is already known
  // (a geocoder's answer, an operator filing), or clicked on the imagery
  // viewer's move-the-dot mode when only the building is known. Both land in
  // site_overrides.csv with the was-value kept. The old refusal to have them
  // in the form assumed nobody arrives holding correct coordinates; the
  // owner arrived holding correct coordinates.
  lat:                    { label: 'Latitude',  hint: 'decimal degrees, e.g. 35.0170106' },
  lon:                    { label: 'Longitude', hint: 'decimal degrees, e.g. -101.3159572' },
...
export function validate(field, value) {
  ...
  if ((field === 'lat' || field === 'lon') && value) {
    if (!/^-?\d+(\.\d+)?$/.test(value)) return 'must be a decimal number';
    const v = +value;
    if (field === 'lat' && (v < -90 || v > 90)) return 'latitude out of range';
    if (field === 'lon' && (v < -180 || v > 180)) return 'longitude out of range';
  }
...
// Everything a real footprint id contains: the parcel ids carry decimal
// APNs (fp-parcel-tx-6804.00000000-...) and the CA/AU vendor refs are
// uppercase - the original [a-z0-9-] class rejected deletes of both.
const FP_ID_RE = /^fp-[\w.-]{1,180}$/;
...
export function footprintError(body, siteById, footprintById, isAsset = () => false) {
  ...
  if (body.sites != null) {
    ...
    for (const id of body.sites) {
      // Plant perimeters and fab shapes attach by their own layer's ids -
      // the plant and fab viewers draw too.
      if (!siteById.has(id) && !isAsset(id)) {
        return `no site, plant or fab with id ${id}`;
      }
    }
  }
...
// ---- plant & fab corrections ------------------------------------------------
// The dot pages' version of the site edit: a second append-only ledger,
// data/asset_overrides.csv, same columns (the site_id column carries the
// asset id - e/g prefixes are plants, f is a fab). Plants and fabs are
// vendor-shaped records, so each flavour edits its own short list of fields:
// identity and geometry, the corrections the registry has actually needed,
// not the site table's whole vocabulary.
export const PLANT_FIELDS = {
  n:   { label: 'Name' },
  own: { label: 'Owner' },
  lat: { label: 'Latitude',  hint: 'decimal degrees' },
  lon: { label: 'Longitude', hint: 'decimal degrees' },
  mw:  { label: 'Capacity, MW', hint: 'number' },
};
export const FAB_FIELDS = {
  n:   { label: 'Name' },
  op:  { label: 'Operator', suggest: true },
  pl:  { label: 'Place' },
  cy:  { label: 'Country', suggest: true, hint: 'ISO 3166-1 alpha-2, e.g. TW' },
  lat: { label: 'Latitude',  hint: 'decimal degrees' },
  lon: { label: 'Longitude', hint: 'decimal degrees' },
};
export const assetFieldsFor = (id) =>
  (String(id)[0] === 'f' ? FAB_FIELDS : PLANT_FIELDS);
// lat/lon/mw arrive as strings and live as numbers on the asset objects -
// distance math downstream would silently break on a string latitude.
const ASSET_NUMERIC = new Set(['lat', 'lon', 'mw']);

export function assetOverridesPath(dataDir) {
  return path.join(dataDir, 'asset_overrides.csv');
}

export function readAssetOverrides(dataDir) {
  const f = assetOverridesPath(dataDir);
  if (!fs.existsSync(f)) return [];
  try {
    return parseCSVObjects(fs.readFileSync(f, 'utf8'))
      .filter(r => r.site_id && assetFieldsFor(r.site_id)[r.field]);
  } catch (err) {
    console.warn('[asset overrides] unreadable, ignoring:', err.message);
    return [];
  }
}

// Applied in place; each asset keeps `edited` so its page can say which
// facts are asserted by hand rather than sourced - same contract as sites.
export function applyAssetOverrides(assets, byId) {
  let n = 0;
  for (const a of assets) {
    const ov = byId.get(a.id);
    if (!ov) continue;
    const table = assetFieldsFor(a.id);
    const edited = a.edited || {};
    for (const [field, rec] of Object.entries(ov)) {
      if (!table[field]) continue;
      edited[field] = { was: a[field] ?? '', ...rec };
      a[field] = ASSET_NUMERIC.has(field) && rec.value !== '' ? +rec.value : rec.value;
      n++;
    }
    a.edited = edited;
  }
...
export function validateAsset(id, field, value) {
  const spec = assetFieldsFor(id)[field];
  if (!spec) return 'not an editable field';
  if (typeof value !== 'string') return 'value must be a string';
  if (value.length > MAX_VALUE) return `value over ${MAX_VALUE} characters`;
  if (field === 'n' && !value.trim()) return 'a name cannot be blank';
  if (field === 'cy' && value && !/^[A-Za-z]{2}$/.test(value)) {
    return 'country must be a two-letter ISO code';
  }
  if (field === 'mw' && value && !/^\d+(\.\d+)?$/.test(value)) {
    return 'capacity must be a number';
  }
  if (field === 'lat' || field === 'lon') {
    // Unlike free-text fields, a blank coordinate is not a retraction - it is
    // a dot with nowhere to stand.
    if (!/^-?\d+(\.\d+)?$/.test(value)) return 'must be a decimal number';
    const v = +value;
    if (field === 'lat' && (v < -90 || v > 90)) return 'latitude out of range';
    if (field === 'lon' && (v < -180 || v > 180)) return 'longitude out of range';
...
export function appendAssetOverrides(dataDir, assetId, asset, edits, note, now) {
  const f = assetOverridesPath(dataDir);
  const fresh = !fs.existsSync(f);
  const lines = [];
  if (fresh) lines.push(COLUMNS.join(','));
  for (const [field, value] of Object.entries(edits)) {
    lines.push([assetId, field, value, asset.lat ?? '', asset.lon ?? '', note || '', now]
      .map(v => csvCell(String(v))).join(','));
  }
  fs.mkdirSync(dataDir, { recursive: true });
  fs.appendFileSync(f, lines.join('\n') + '\n');
