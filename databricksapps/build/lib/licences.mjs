...
export const LICENCES = [
  ...
  // ---- government works ---------------------------------------------------
  { grp: 'Public domain / government works', source: 'US EIA (Form 860M, and the annual Form 860 for plant addresses)',
    used: 'United States power plants; street address, city, ZIP and water source from the annual Schedule 2',
    ...
    where: 'src/plants.py' },
  { grp: 'Public domain / government works', source: 'US Census Bureau (Gazetteer counties and ZCTA files)',
    used: 'Legal names of US county-equivalents on plant pages (parish, borough, independent city, municipio); ZCTA centres to reject a reported ZIP that is nowhere near the plant',
    licence: 'US Government work — public domain',
    url: 'https://www.census.gov/geographies/reference-files/time-series/geo/gazetteer-files.html',
    credit: 'US Census Bureau', obliges: 'Nothing required.',
    where: 'src/plants.py' },
  ...
  // ---- US county and state parcel layers ------------------------------------
  { grp: 'US public parcel records', source: 'County assessors and state GIS offices, via their own ArcGIS REST services',
    used: 'The lot under each exactly-located US site as its campus boundary, with the '
        + 'assessor\'s parcel id, owner name and acreage; grown to the same owner\'s touching lots',
    licence: 'Public records. Terms vary by publisher: most state aggregates are public domain or '
           + 'open-data licensed; some county services ask for attribution or forbid bulk resale.',
    url: '',
    credit: 'Parcel boundaries: county assessors and state GIS offices (see data/parcel_layers.json)',
    obliges: 'Attribute the publishing office where it asks (the registry records each publisher). '
           + 'These are point queries, one lot per site, never a bulk copy of a layer. Owner '
           + 'names are what the county publishes; where a county withholds them the field is '
           + 'blank rather than filled from elsewhere.',
    where: 'src/parcels.py, data/parcel_layers.json' },
  ...
  <section class="panel card">
    ...
    <div class="tablewrap"><table>
      ...
      ${g.rows.map(r => `<tr>
        ...
        <td>${esc(r.used)}</td>
        <td>${r.url ? `<a href="${esc(r.url)}" target="_blank" rel="noopener noreferrer">${esc(r.licence)}</a>`
                    : esc(r.licence)}
