...
import { esc, n0, makeGeo, usdM } from './summary.mjs';
...
function roll(list) {
  ...
  return {
    ...
    // Announced investment (US$M), same denominator rule - and the same
    // caveat as any sum of announcements: some figures cover a campus, some
    // one phase, so the total is "what has been announced here", not a
    // balance-sheet quantity.
    inv: inv.reduce((a, f) => a + f.inv, 0),
    invOf: `${inv.length} of ${list.length}`,
...
export function renderFabsPage(fabs, regions, outlined = new Set()) {
  ...
  const regionRows = [...byRegion.entries()]
    ...
    .map(([name, list]) => {
      ...
      return `<tr>
        ...
        <td class="num">${s.inv ? usdM(s.inv) : ''}<span class="dim"> ${s.inv ? esc(s.invOf) : ''}</span></td>
  ...
  // ---- every fab -----------------------------------------------------------
  // outlined/unmapped are deliberately non-overlapping strings, so typing
  // either into the filter box shows exactly that half of the layer.
  const outCell = (id) => (outlined.has(id)
    ? '<td>outlined <span class="dim">✓</span></td>'
    : '<td class="dim">unmapped —</td>');
  const fabRows = fabs.map(f => `<tr>
    <td><a href="/maps/fab/${esc(f.id)}"><b>${esc(f.n)}</b></a>${[f.nt, f.pn].filter(Boolean)
    ...
    <td class="num">${f.inv ? usdM(f.inv) : '<span class="dim">—</span>'}</td>
    ${outCell(f.id)}
...
<header class="topbar">
  ...
  <!-- This nav is a COPY of the one in summary.mjs page(), and it had already
       drifted: /power was added there and never here, so the fabs page was the
       one page you could not reach the dependency layer from. Kept in step by
       hand for now; the real fix is this page using the shared header. -->
...
<main class="wrap">
  <header class="sitehead">
    ...
    <p class="note"><a href="/maps?fabs=1">Show them on the map</a></p>
  </header>
  ...
  <section class="panel card">
    <div class="fabtiles">
      ...
      ${tile('Announced investment', all.inv ? usdM(all.inv) : '—', all.invOf + ' fabs')}
    </div>
    ...
    <p class="note"><b>Announced $</b> is the opposite kind of figure: an
    investment somebody announced, with its source kept per record — never a
    model. But announcements draw their own boundaries: some cover a whole
    campus, some one phase, and the per-fab note says which. A sum of them is
    "what has been announced here", not a balance sheet.</p>
  </section>
  ...
  <section class="panel card">
    ...
    <div class="tablewrap"><table>
      <thead><tr><th>Region</th><th class="num">Fabs</th><th class="num">Operating</th>
        ...
        <th class="num">Est. MW</th><th class="num">Announced $</th><th>Countries</th></tr></thead>
  ...
  <section class="panel card">
    ...
    <div class="tablewrap"><table>
      <thead><tr><th>Company</th><th class="num">Fabs</th><th class="num">Finest</th>
        <th class="num">Est. MW</th><th class="num">Announced $</th><th>Where</th></tr></thead>
      <tbody>${coRows}</tbody>
    </table></div>
    <p class="note"><b>Finest</b> is the smallest process node in production. A dimmed figure marked * is not running yet — it is the finest node announced or under construction, shown only where nothing is in production to report.</p>
  </section>
  ...
  <section class="panel card">
    ...
    <div class="tablewrap"><table>
      <thead><tr><th>Fab</th><th>Operator</th><th>Where</th><th>Status</th>
        ...
        <th class="num">Est. MW</th><th class="num">Announced $</th><th>Footprint</th><th>Source</th></tr></thead>
      <tbody>${fabRows}</tbody>
    </table></div>
