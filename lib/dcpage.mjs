...
function roll(list) {
  ...
  return {
    ...
    outl: list.filter(s => OUTLINED.has(s.id)).length,
  };
}
...
export function renderDcPage(sites, regions, operators, outlined = new Set()) {
  OUTLINED = outlined;
  ...
  const regionRows = [...byReg.entries()]
    ...
    .map(([name, list]) => {
      ...
      return `<tr>
        ...
        <td class="num">${r.outl}<span class="dim"> ${Math.round(r.outl / r.n * 100)}%</span></td>
  ...
  const byOp = new Map();
  for (const s of sites) {
    const k = s.ok || s.o || '(unresolved)';
    if (!byOp.has(k)) byOp.set(k, []);
    byOp.get(k).push(s);
  }
  const opsSorted = [...byOp.entries()].sort((a, b) => b[1].length - a[1].length);
  const TOP = 40;
  const opRows = opsSorted.slice(0, TOP).map(([k, list]) => {
    const r = roll(list);
    // A resolved key names the directory entry; a raw-only group shows its
    // most common spelling and no link, since no page exists for it.
    const dir = operators?.find(o => o.key === k);
    const name = dir?.name
      || [...list.reduce((m, s) => m.set(s.o, (m.get(s.o) || 0) + 1), new Map())]
           .sort((a, b) => b[1] - a[1])[0]?.[0]
      || '(unresolved)';
    const label = dir
      ? `<a href="/operator/${encodeURIComponent(k)}"><b>${esc(name)}</b></a>`
      : `<b>${esc(name)}</b>`;
  ...
  const fields = [
    ...
    ['A building outline', s => OUTLINED.has(s.id)],
  ];
  ...
  const mwRows = byMw.map(s => `<tr>
    <td><a href="/maps/site/${esc(s.id)}"><b>${esc(s.n || s.o || 'Data centre')}</b></a></td>
    <td>${esc(s.o || '')}</td>
  ...
  <section class="panel card">
    ...
    ${table(['Region', { n: 'Sites' }, { n: 'AI' }, { n: 'Operators' },
             { n: 'Outlined' }, { n: 'MW' }, { n: 'Floor area ft²' }, 'Countries'], regionRows)}
  </section>

  <section class="panel card">
    <h2>By operator <span class="dim">${n0(opsSorted.length)}</span></h2>
    ${table(['Operator', { n: 'Sites' }, { n: 'AI' }, { n: 'Countries' },
             { n: 'Outlined' }, { n: 'MW' }, { n: 'Floor area ft²' }, 'Regions'], opRows,
            `Showing the ${TOP} largest of ${n0(opsSorted.length)}.
             <a href="/maps?operators=1">The full directory</a> has the rest.`)}
  </section>
