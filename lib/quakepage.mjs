...
// plants, 216 fabs, all of them true until the next ingest. An earthquake is an
...
const KIND_HREF = { dc: '/maps/site/', plant: '/maps/plant/', fab: '/maps/fab/' };
...
export function renderQuakesPage(quakes) {
  ...
  // The magnitude floor is an argument to src/quakes.py, not a constant, so
  // it is read back off the events rather than written down here - a page
  // claiming M5+ over an M4+ pull is worse than one that says nothing.
  const floor = quakes.length
    ? `M${Math.min(...quakes.map(q => q.mag)).toFixed(1)}+` : 'M5+';
  ...
  const tiles = [
    tile('Events', n0(quakes.length), `USGS ${floor}, trailing 90 days`),
  ...
  return page({
    title: 'Earthquakes',
    crumb: 'Earthquakes',
    lede: `${n0(quakes.length)} USGS ${floor} events in the trailing 90 days.
