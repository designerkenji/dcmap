...
export async function loadAll() {
  ...
  // US power plants at or above 100 MW, from EIA-860M via src/plants.py. Not a
  // worldmonitor config, so it is read straight from data/ rather than through
  // the vendoring path the zone layers use.
  ...
  console.log(`data: ${sites.length} sites (${mapped} mapped, ${sites.length - mapped} search-only), ` +
    ...
    `${countries.length} countries, ${plants.length} power plants`);
  ...
  return {
    ...
    overridesById, childrenBySite, kmBetween,
  };
}
