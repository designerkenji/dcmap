...
// `root` is the repo checkout. It is what git is run in and what the pipeline
// scripts are spawned from, so it is deliberately NOT derived from DATA: in a
// bundled deploy there is no checkout and no src/ to run, and `bundled` is the
// flag that tells /data to say so rather than shelling out into nothing.
export const paths = { data: DATA, root: ROOT, bundled };
