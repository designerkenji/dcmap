...
export const STEPS = [
  ...
  { key: 'quakes', script: 'quakes.py', label: 'Earthquakes',
    ...
    // NOT data/quakes.json. This one writes into the worldmonitor checkout
    // NEXT DOOR, which this repo does not version - so its output is invisible
    // to /data's git section by construction, and the page says so.
    writes: ['worldmonitor/src/config/quakes-recent.ts', 'data/raw/quake_*/'],
    about: 'USGS, rolling window — old events leave as new ones arrive. Writes '
         + 'into the worldmonitor repo next door, which git here does not track.' },
  ...
  { key: 'power_lines', script: 'power_lines.py', label: 'Transmission corridors (OSM)',
    group: 'Sources', cost: 'free',
    writes: ['data/transmission_lines.geojson', 'data/raw/power_lines_<region>.json'],
...
export function stepFreshness(root) {
  const out = [];
  for (const s of STEPS) {
    ...
    for (const w of s.writes || []) {
      ...
      let files = [];
      if (star === -1) {
        files = [path.join(root, pat)];
      } else {
        const dir = path.join(root, pat.slice(0, pat.lastIndexOf('/')));
...
export function startRun(root, key, logDir) {
  ...
  const proc = spawn('python3', [path.join('src', step.script)], {
