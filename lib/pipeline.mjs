...
export const STEPS = [
  ...
  { key: 'plants', script: 'plants.py', label: 'Power plants',
    group: 'Sources', cost: 'free', writes: ['data/power_plants.json', 'data/power_plants_meta.json'],
    about: 'EIA-860M for the US (street address and water source from the annual EIA-860, '
         + 'county names from the Census Gazetteer), Global Energy Monitor for the rest.' },
  ...
  { key: 'parcels', script: 'parcels.py', label: 'County parcels',
    group: 'Sources', cost: 'free',
    writes: ['raw/parcels.geojson', 'raw/parcels_point/', 'raw/site_county.json'],
    about: 'The lot under each exactly-located dot, from the county or state parcel '
         + 'layer that covers it (data/parcel_layers.json plus the hand-wired list), '
         + 'grown to the same owner\'s touching lots. Sites are taken at their '
         + 'hand-corrected coordinate. Run Footprints after it to merge.' },
