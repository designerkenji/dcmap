    ...
    ${countries.map(([cname, rows]) => `
      ...
      <ul class="opx-sites">
        ${rows.map(s => {
          ...
          return `<li>
            <a class="opx-name" href="/maps/site/${encodeURIComponent(s.site_id)}">${esc(name)}</a>
...
<header class="topbar">
  ...
  <a class="themebtn searchbtn" href="/maps?search=1" aria-label="Search the registry" title="Search"></a>
...
<main class="wrap">
  <header class="sitehead">
    <nav class="crumbs"><a href="/">Data Centres</a> <span>›</span>
      <a href="/maps?operators=1">Operators</a> <span>›</span> <b>${esc(op.name)}</b></nav>
    <div class="opx-hero">
      ${logo(op, profile)}
      <div>
        <h1>${esc(op.name)}</h1>
    ...
    <p class="note">
      <a href="/maps?op=${encodeURIComponent(op.key)}">Show all ${sites.length} on the map</a>
