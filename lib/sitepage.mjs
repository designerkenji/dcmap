...
export function renderSitePage(ctx) {
  const { site, rows, va, traj, timeline, epoch, images = [],
          ...
          parent = null, children = [], plant = null, footprints = [] } = ctx;
  ...
  <section class="panel card" id="supply">
    ...
    ${plant && st ? `
      <div class="facts">
        ${fact('Plant', `<strong>${esc(plant.n)}</strong>`)}
        ...
        ${fact('Where', esc([plant.st, plant.cy].filter(Boolean).join(' · ')))}
  ...
  <section class="panel card" id="wayback">
    ...
    <div class="wb" id="wb" hidden>
      ...
      <div class="wb-kind">
        <span>This dot is</span>
        <span class="seg" id="wb-kindseg">${KINDS.map(k =>
          `<button type="button" data-k="${k}" aria-pressed="${k === kindOf(site)}">${k}</button>`).join('')}</span>
        <button type="button" class="tb-btn" id="wb-near">Show nearby dots</button>
        <button type="button" class="tb-btn" id="wb-fp">Show footprints</button>
        <span class="note" id="wb-kmsg"></span>
        <span class="wb-legend" id="wb-legend" hidden>
          <i class="lg-point"></i><span>unknown</span>
          <i class="lg-building"></i><span>building</span>
          <i class="lg-campus"></i><span>campus</span>
        </span>
      </div>
      <div class="wb-sel" id="wb-sel" hidden></div>
      <div class="wb-bar">
...
<main class="wrap">
  ...
  <section class="panel card" id="sitecard">
    ...
    ${parent || children.length ? `<div class="note edited-note">
      ${parent ? `Part of <a href="/site/${esc(parent.site_id)}">${esc(
      ...
      ${children.length ? `<b>${children.length}</b> building${children.length === 1 ? '' : 's'}
        on this campus:<ul class="tree">${children.map(c =>
          `<li><a href="/site/${esc(c.site_id)}">${esc(c.name || c.epoch_name || c.site_id)}</a>
...
(async () => {
  ...
  // ---- finding the captures, here rather than on the server ---------------
  // A release is a snapshot of the WHOLE WORLD, but almost none of the world
  // changes between two of them. Ask for one tile from all 195 releases and
  // you get the same handful of pictures over and over - at Quincy, 195
  // releases are 14 actual captures. So they are sieved with the service's
  // own tilemap endpoint:
  //
  //   .../tilemap/{release}/{z}/{y}/{x}/1/1  ->  { data: [1], select: [10] }
  //
  ...
  //
  // This ran on the server until both endpoints turned out to send
  // Access-Control-Allow-Origin: *, which made the proxy pure cost: 195
  // outbound requests from one machine while a reader waited on a blank
  // panel. In the browser the same scan gets two caches the server never had.
  // The tilemap replies carry Cache-Control: max-age=86400, so the HTTP cache
  // absorbs a re-scan for a day; and the sieved answer goes in localStorage
  // under its TILE, not its site, so every other site in the same tile is
  // free - which was the one thing the old in-process cache did well, except
  // that it died with the process and was never shared between readers.
  const CONFIG_URL = 'https://s3-us-west-2.amazonaws.com/config.maptiles.arcgis.com'
                   + '/waybackconfig.json';
  const TILEMAP = 'https://wayback.maptiles.arcgis.com/arcgis/rest/services/world_imagery'
                + '/wmts/1.0.0/default028mm/MapServer/tilemap';
  // 17 is ~1.2 m/px at the equator: a data centre hall is tens of pixels
  // across and a new building between two frames is unmistakable. Finer would
  // narrow the frame past the campus and find dates the archive has none for.
  const SCAN_Z = 17;
  // Enough in flight to keep the scan a few seconds, few enough not to look
  // like an attack. It is 195 requests either way.
  const WORKERS = 12;

  const nz = Math.pow(2, SCAN_Z), sr = SITE_LAT * Math.PI / 180;
  const TX = Math.floor((SITE_LON + 180) / 360 * nz);
  const TY = Math.floor((1 - Math.log(Math.tan(sr) + 1 / Math.cos(sr)) / Math.PI) / 2 * nz);
  const CKEY = 'wb1:' + SCAN_Z + '/' + TX + '/' + TY;

  async function scan() {
    try {
      const hit = JSON.parse(localStorage.getItem(CKEY) || 'null');
      if (hit && Array.isArray(hit.caps)) return hit.caps;
    } catch (err) { /* private mode, or someone else's key. Just scan. */ }

    const cfg = await (await fetch(CONFIG_URL)).json();
    const rels = [];
    for (const item of Object.values(cfg)) {
      // Doubled backslashes: this script is inside a template literal, so a
      // lone \\d reaches the browser as a bare d and the second regex becomes
      // a line comment. Same trap as the no-backticks rule above.
      const d = (item.itemTitle || '').match(/\\d{4}-\\d{2}-\\d{2}/);
      const rel = (item.itemURL || '').match(/\\/tile\\/(\\d+)\\//);
      if (d && rel) rels.push({ release: +rel[1], date: d[0] });
    }
    // Oldest first, because the sieve keeps the FIRST release to show a given
    // picture and that only means "when it entered the archive" in date order.
    rels.sort((a, b) => (a.date < b.date ? -1 : 1));
    if (!rels.length) throw new Error('the release list parsed to nothing');

    const probed = new Array(rels.length);
    let next = 0, done = 0;
    async function worker() {
      while (next < rels.length) {
        ...
        const rel = rels[i];
        try {
          const r = await fetch(TILEMAP + '/' + rel.release + '/' + SCAN_Z
                              + '/' + TY + '/' + TX + '/1/1');
          if (r.ok) {
            const j = await r.json();
            if (j.data && j.data[0]) {
              const from = (j.select && j.select[0] != null) ? j.select[0] : rel.release;
              probed[i] = { release: rel.release, date: rel.date, from: from };
            }
          }
        } catch (err) { /* one dead probe is a missing frame, not a dead scan */ }
        done++;
        msg.textContent = 'Looking through the archive… ' + done + ' of ' + rels.length;
      }
    }
    const crew = [];
    for (let i = 0; i < Math.min(WORKERS, rels.length); i++) crew.push(worker());
    await Promise.all(crew);

    const seen = Object.create(null), out = [];
    for (const p of probed) {
      if (!p || seen[p.from]) continue;
      seen[p.from] = 1;
      out.push({ release: p.release, date: p.date });
    }
    // Best effort. A full quota must not cost the reader the scan they just
    // waited for, and the HTTP cache still makes the next one cheap.
    try { localStorage.setItem(CKEY, JSON.stringify({ caps: out })); } catch (err) { /* full */ }
    return out;
  }
  ...
  let Z = SCAN_Z, dx = 0, dy = 0, at = caps.length - 1;
  ...
  function redraw() {
    layout(); paint();
    if (typeof drawNear === 'function') drawNear();
    if (typeof drawFp === 'function') drawFp();
  }
  ...
  // ---- the neighbours ------------------------------------------------------
  // Same coordinate arithmetic as the tiles: a neighbour's offset inside the
  // grid is its world pixel minus this site's, so it pans and zooms with the
  // ground for free and never has to be repositioned.
  const nbLayer = document.createElement('div');
  nbLayer.className = 'wb-nblayer';
  grid.appendChild(nbLayer);
  const nearBtn = document.getElementById('wb-near');
  const kmsg = document.getElementById('wb-kmsg');
  const legend = document.getElementById('wb-legend');
  const selBar = document.getElementById('wb-sel');
  let near = null, picked = null;

  const pxOf = (lat, lon) => {
    const r = lat * Math.PI / 180;
    return {
      x: ((lon + 180) / 360) * Math.pow(2, Z) * 256 - sitePx(Z),
      y: ((1 - Math.log(Math.tan(r) + 1 / Math.cos(r)) / Math.PI) / 2)
         * Math.pow(2, Z) * 256 - sitePy(Z),
    };
  };
  ...
  function drawNear() {
    ...
    if (!near) return;
    for (const n of near) {
      const q = pxOf(n.lat, n.lon);
      const b = document.createElement('button');
      b.type = 'button';
      b.className = 'wb-nb k-' + n.kind + (picked === n.id ? ' on' : '');
      b.style.left = q.x + 'px';
      b.style.top = q.y + 'px';
      b.title = (n.name || n.id) + ' — ' + n.kind + ', ' + n.km + ' km';
      b.addEventListener('click', (e) => { e.stopPropagation(); pick(n); });
      nbLayer.appendChild(b);
    }
  }

  // A link between two dots can be made from either end, and which end you are
  // standing on decides the sentence. On a campus looking at a building you
  // want to ADOPT it; on a building looking at a campus you want to JOIN it.
  // Offering only the second meant that to say "that orange dot belongs to
  // this campus" you had to leave the page, open the orange dot and link
  // backwards - which is the same fact entered from the far end.
  //
  // Only actions that would be allowed are offered. Every rule the server
  // enforces is knowable here, so a button that would come back as an error is
  // a disabled button with the reason on it instead.
  function actionsFor(n) {
    const out = [];

    // Downward: the neighbour becomes a building of this campus.
    if (n.parent === SITE_ID) {
      out.push({ label: 'Remove from this campus', target: n.id,
                 fields: { parent_site_id: '' } });
    } else if (SITE_KIND === 'campus') {
      if (n.children) {
        out.push({ label: 'It has buildings of its own', disabled: true,
                   title: 'A site with buildings cannot also be part of a campus.' });
      } else if (n.kind === 'point') {
        // Says "as a building" because it sets the kind too. Doing that
        // silently would be putting words in somebody's mouth.
        out.push({ label: 'Add as a building', target: n.id,
                   fields: { parent_site_id: SITE_ID, site_kind: 'building' } });
      } else {
        out.push({ label: n.parent ? 'Move to this campus' : 'Add to this campus',
                   target: n.id, fields: { parent_site_id: SITE_ID } });
      }
    }

    // Upward: this site becomes a building of the neighbour.
    if (SITE_PARENT === n.id) {
      out.push({ label: 'Unlink from this campus', target: SITE_ID,
                 fields: { parent_site_id: '' } });
    } else if (n.kind === 'campus' && !SITE_CHILDREN) {
      out.push({ label: SITE_PARENT ? 'Move us to this campus' : 'Make this our campus',
                 target: SITE_ID, fields: { parent_site_id: n.id } });
    }

    if (!out.length) {
      out.push({ label: SITE_KIND === 'campus' ? 'Nothing to link here'
                                               : 'Mark this dot a campus to adopt it',
                 disabled: true,
                 title: SITE_KIND === 'campus'
                   ? 'This neighbour cannot be linked either way.'
                   : 'A building can only belong to a site marked as a campus — '
                     + 'either mark this dot a campus, or mark that one and join it.' });
    }
    return out;
  }
  ...
  function pick(n) {
    ...
    info.className = 'grow';
    info.innerHTML = '<b></b> <span class="kinbadge k-' + n.kind + '"></span> '
                   + '<span class="dim"></span>';
    info.querySelector('b').textContent = n.name || n.id;
    info.querySelector('.kinbadge').textContent = n.kind;
    info.querySelector('.dim').textContent =
      (n.operator ? n.operator + ' · ' : '') + n.km + ' km away'
      + (n.children ? ' · ' + n.children + ' building(s)' : '')
      + (n.parent === SITE_ID ? ' · on this campus' : '');
    selBar.appendChild(info);

    const open = document.createElement('a');
    open.className = 'tb-btn';
    open.href = '/site/' + encodeURIComponent(n.id);
    open.textContent = 'Open page';
    selBar.appendChild(open);

    for (const a of actionsFor(n)) {
      const b = document.createElement('button');
      b.type = 'button';
      b.className = 'tb-btn';
      b.textContent = a.label;
      if (a.disabled) { b.disabled = true; b.title = a.title || ''; }
      else b.addEventListener('click', () => saveFields(a.fields, a.target));
  ...
  // ---- the outlines --------------------------------------------------------
  // The dots say where; the footprints say what shape, and over a dated
  // archive that is the more interesting claim: an outline sitting on bare
  // earth in 2019 and on a roof in 2023 dates the building it describes, and
  // one that never lines up with anything is a footprint worth correcting.
  //
  // Drawn as ONE svg re-fitted to the features each redraw, rather than a
  // fixed canvas. The layer has to survive zoom 15 to 20, where the same
  // campus is 40 px across or 3,000, and any fixed extent is wrong at one end.
  const SVGNS = 'http://www.w3.org/2000/svg';
  const fpSvg = document.createElementNS(SVGNS, 'svg');
  fpSvg.setAttribute('class', 'wb-fplayer');
  grid.insertBefore(fpSvg, nbLayer);      // dots stay clickable above outlines
  const fpBtn = document.getElementById('wb-fp');
  let fps = null, fpPicked = null;

  function drawFp() {
    while (fpSvg.firstChild) fpSvg.removeChild(fpSvg.firstChild);
    if (!fps || !fps.length) { fpSvg.style.display = 'none'; return; }
    fpSvg.style.display = '';
    // Project every ring once, and take the extent while we are there.
    let x0 = Infinity, y0 = Infinity, x1 = -Infinity, y1 = -Infinity;
    const shapes = fps.map(f => {
      const rings = f.rings.map(r => r.map(c => {
        const q = pxOf(c[1], c[0]);
        if (q.x < x0) x0 = q.x; if (q.x > x1) x1 = q.x;
        if (q.y < y0) y0 = q.y; if (q.y > y1) y1 = q.y;
        return q;
      }));
      return { f: f, rings: rings };
    });
    if (!Number.isFinite(x0)) { fpSvg.style.display = 'none'; return; }
    const pad = 6;
    fpSvg.style.left = (x0 - pad) + 'px';
    fpSvg.style.top = (y0 - pad) + 'px';
    fpSvg.setAttribute('width', (x1 - x0) + pad * 2);
    fpSvg.setAttribute('height', (y1 - y0) + pad * 2);
    for (const s of shapes) {
      const path = document.createElementNS(SVGNS, 'path');
      path.setAttribute('d', s.rings.map(r =>
        'M' + r.map(q => (q.x - x0 + pad).toFixed(1) + ',' + (q.y - y0 + pad).toFixed(1))
          .join('L') + 'Z').join(' '));
      // Even-odd, so a ring inside a ring reads as a courtyard rather than
      // being painted over - the same rule the pipeline uses to decide
      // whether a dot is inside a shape.
      path.setAttribute('fill-rule', 'evenodd');
      path.setAttribute('class', 'wb-fp k-' + s.f.kind
        + (s.f.mine ? ' mine' : '') + (fpPicked === s.f.id ? ' on' : ''));
      const what = s.f.kind === 'campus' ? 'campus boundary' : 'building footprint';
      path.appendChild(document.createElementNS(SVGNS, 'title'))
        .textContent = (s.f.name || s.f.op || what) + ' — ' + what
          + (s.f.m2 ? ', ' + Math.round(s.f.m2).toLocaleString() + ' m²' : '');
      path.addEventListener('click', (e) => { e.stopPropagation(); pickFp(s.f); });
      fpSvg.appendChild(path);
    }
  }

  // Selecting an outline reuses the neighbour bar, because it answers the same
  // question in the same place: here is a thing near this dot, here is what is
  // known about it, here is what you can do with it.
  function pickFp(f) {
    fpPicked = f.id;
    picked = null;
    selBar.hidden = false;
    ...
    const info = document.createElement('span');
    info.className = 'grow';
    info.innerHTML = '<b></b> <span class="kinbadge k-' + f.kind + '"></span> '
                   + '<span class="dim"></span>';
    const what = f.kind === 'campus' ? 'campus boundary' : 'building footprint';
    info.querySelector('b').textContent = f.name || f.op || what;
    info.querySelector('.kinbadge').textContent = f.kind;
    info.querySelector('.dim').textContent =
      (f.src === 'im3' ? 'IM3 atlas'
       : f.src === 'osm' ? 'OpenStreetMap'
       : f.src === 'osm-site' ? 'OSM — large building here, not labelled a data centre'
       : f.src === 'derived' ? 'derived — hull of this site’s halls, not surveyed'
       : f.src === 'parcel' ? 'county parcel record'
       : 'drawn by hand')
      + (f.m2 ? ' · ' + Math.round(f.m2).toLocaleString() + ' m²' : '')
      + (f.mine ? ' · this dot' : '');
    selBar.appendChild(info);

    // The whole point of showing the name here: the outline round a dot very
    // often knows what the building is called when the registry only has the
    // operator. Same offer as the edit form, taken from where you are looking.
    const nm = (f.name || '').trim();
    if (nm && nm !== (SITE_NAME || '').trim()) {
      const b = document.createElement('button');
      b.type = 'button';
      b.className = 'tb-btn';
      b.textContent = 'Name this dot “' + nm + '”';
      b.addEventListener('click', () => saveFields({ name: nm }));
      selBar.appendChild(b);
    }
    drawFp();
    drawNear();
  ...
  fpBtn.addEventListener('click', async () => {
    if (fps) { fps = null; fpPicked = null; selBar.hidden = true; drawFp();
               fpBtn.textContent = 'Show footprints'; return; }
    fpBtn.disabled = true; kmsg.textContent = 'looking…';
    try {
      const r = await fetch('/api/footprints/' + encodeURIComponent(SITE_ID) + '?km=1.5');
      const out = await r.json();
      fps = out.near || [];
      const mine = fps.filter(f => f.mine).length;
      kmsg.textContent = fps.length
        ? fps.length + ' outline' + (fps.length === 1 ? '' : 's') + ' within 1.5 km'
          + (mine ? ', ' + mine + ' on this dot — click one' : ' — none on this dot')
        : 'no footprints mapped within 1.5 km';
      fpBtn.textContent = 'Hide footprints';
      drawFp();
    } catch (err) { kmsg.textContent = 'failed: ' + err.message; }
    fpBtn.disabled = false;
  });

  nearBtn.addEventListener('click', async () => {
    if (near) { near = null; picked = null; selBar.hidden = true; drawNear();
                legend.hidden = true; kmsg.textContent = '';
                nearBtn.textContent = 'Show nearby dots'; return; }
    nearBtn.disabled = true; kmsg.textContent = 'looking…';
    try {
      const r = await fetch('/api/nearby/' + encodeURIComponent(SITE_ID) + '?km=3');
      const out = await r.json();
      near = out.near || [];
      kmsg.textContent = near.length
        ? near.length + ' within 3 km — click one'
        : 'no other registry sites within 3 km';
      nearBtn.textContent = 'Hide nearby dots';
      legend.hidden = !near.length;
      drawNear();
    } catch (err) { kmsg.textContent = 'failed: ' + err.message; }
  ...
  // ---- what this dot is ----------------------------------------------------
  // The target is which SITE is being changed, and it is not always the one
  // whose page this is: adopting a neighbour writes the link onto the
  // neighbour. (No backticks in here - this script is inside a template
  // literal, and one would end the string.)
  async function saveFields(fields, target) {
    kmsg.textContent = 'saving…';
    try {
      const r = await fetch('/api/site/' + encodeURIComponent(target || SITE_ID), {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ fields: fields }),
      });
      const out = await r.json();
      if (!r.ok) { kmsg.textContent = out.error || ('failed: ' + r.status); return; }
      location.reload();
    } catch (err) { kmsg.textContent = 'failed: ' + err.message; }
  }
  document.getElementById('wb-kindseg').addEventListener('click', (e) => {
    const b = e.target.closest('button[data-k]');
    if (b && b.getAttribute('aria-pressed') !== 'true') saveFields({ site_kind: b.dataset.k });
  });
