...
export async function gitChanges(root, since, subdir = 'data') {
  const out = await run('git', [
    ...
    // Two pathspecs for one tree: `subdir` is relative to root (dcmap/data),
    // and :(top)data is the same tree at the path it had before the whole
    // data folder moved into dcmap/ - so the page does not go blank for a
    // month while its window catches up with the rename.
    '--', subdir, `:(top)${subdir}`,
  ], root);
...
export const BLIND_SPOTS = [
  ...
  // Two blind spots that used to sit here are closed, and it is worth saying
  // so rather than deleting them silently: the worldmonitor layer configs are
  // now written to data/wm/ inside this repository, where every git query on
  // this page can see them; and the bundled copy under dcmap/data/ no longer
  // exists, because dcmap/data/ IS the data.
